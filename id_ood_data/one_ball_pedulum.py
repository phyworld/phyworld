...
import imageio
from Box2D import *
from Box2D.b2 import (world, polygonShape, staticBody, dynamicBody, revoluteJointDef, circleShape)
import torch
import random
import h5py
...
def simulate_pedulum(
    ...
    # Colors
    colors = {staticBody: (0, 0, 0, 255),  # Black for static body
              dynamicBody: (255, 0, 0, 255)} # Red for dynamic body

    world = b2World(gravity=(0, -9.8), doSleep=False)
    anchor = world.CreateBody(position=(world_scale/2, world_scale * 0.9), 
        shapes=polygonShape(box=(0.05, 0.05)),  type=staticBody)

    x = l1 * np.sin(theta / 180 * np.pi)
    y = l1 * np.cos(theta / 180 * np.pi)

    pendulum_bob = world.CreateBody(position=(world_scale/2 - x, world_scale * 0.9 - y), type=dynamicBody)
    pendulum_bob.CreateFixture(shape=circleShape(radius=r1, pos=b2Vec2(0, 0)), density=1.0)
    ...
    pendulum_bob.angularVelocity = 0
    ...
    for i in range(numsteps):
        ...
        # Draw anchor
        anchor_pos = (anchor.position.x * img_width / world_scale, img_height - anchor.position.y * img_height / world_scale)
        pygame.draw.circle(screen, colors[b2_staticBody], anchor_pos, 2)
        
        # Draw pendulum bob
        bob_pos = (pendulum_bob.position.x * img_width / world_scale, img_height - pendulum_bob.position.y * img_height / world_scale)
        # pygame.draw.circle(screen, colors[b2_dynamicBody], bob_pos, int(r1 * img_width / world_scale))

        radius = round(r1 * img_width / world_scale)
        pygame.gfxdraw.filled_circle(screen, round(bob_pos[0]), round(bob_pos[1]), radius, colors[b2_dynamicBody])
        pygame.gfxdraw.aacircle(screen, round(bob_pos[0]), round(bob_pos[1]), radius, colors[b2_dynamicBody])

        # Draw the pendulum rod or string
        bob_edge_pos = (anchor_pos[0] + (bob_pos[0] - anchor_pos[0]) * (l1 - r1) / l1, 
                            anchor_pos[1] + (bob_pos[1] - anchor_pos[1]) * (l1 - r1) / l1)
        # pygame.draw.line(screen, (0, 0, 0), anchor_pos, bob_edge_pos, 4)
        pygame.draw.aaline(screen, (0, 0, 0), anchor_pos, bob_edge_pos, blend=1)
        ...
        if i % stride == 0:
            ...
            frames.append(np.transpose(buffer, (1, 0, 2)))
            # Record bob position
            bob_pos = pendulum_bob.position
            bob_positions.append((bob_pos.x, world_scale - bob_pos.y))

    positions = np.array(bob_positions)
    radii = np.array([])
    # print(positions)

    return np.array(frames, dtype=np.uint8), positions, True
...
def generate_videos_hdf5(scenes, scenes_id, data_dir='./', seed=42, store_video=False):
    ...
    for scene in scenes:
        r1, l1, theta1 = scene
        frames, positions, success = simulate_pedulum(r1, l1, theta1, world_scale=WORLD_SCALE, stride=STRIDE, numsteps=STRIDE*NUM_FRAMES)
        if not success: continue
        batched_images.append(frames)
        batched_positions.append(positions)
        batched_init.append([r1, l1, theta1])

        if store_video:
            video_dir = Path(data_dir) / 'vis_videos'
            if not os.path.exists(video_dir):
                os.mkdir(video_dir)
            mp4_path = video_dir / f'{r1:.3f}_{l1:.3f}_{theta1:.1f}.mp4'
            # print(mp4_path)
...
def generate_combinations(r1_list, l1_list, theta1_list):
    ...
    for r1 in r1_list:
        for l1 in l1_list:
            for theta in theta1_list:
                ...
                combinations.append((r1, l1, theta))
...
def in_dist_generate():
    ...
    if args.data_size_level == 0:
        ...
        R_SPACE = 3
        THETA_SPACE = 3
    l_list = np.linspace(MIN_L, MAX_L, L_SPACE)
    r_list = np.linspace(MIN_R, MAX_R, R_SPACE)
    theta_list = np.linspace(MIN_THETA, MAX_THETA, THETA_SPACE)

    if not args.data_for_vis:
        # V_SPACE= 15
        l_list = sorted([uniform_random(MIN_L, MAX_L) for _ in range(L_SPACE)])
        r_list = sorted([uniform_random(MIN_R, MAX_R) for _ in range(R_SPACE)])
        theta_list = sorted([uniform_random(MIN_THETA, MAX_THETA) for _ in range(THETA_SPACE)])
    else:
        l_list = np.linspace(MIN_L, MAX_L, 3)
        r_list = np.linspace(MIN_R, MAX_R, 3)
        theta_list = np.linspace(MIN_THETA, MAX_THETA, 3)

    # List to hold all combinations
    combinations = generate_combinations(r_list, l_list, theta_list)
...
def out_dist_generate_xl():
    ...
    if not args.data_for_vis:
        ...
        v_ood_list = np.array([4.5, 6.0])
        r_ood_list = np.array([1.5, 2.0])
    # print(v_id_list, r_id_list, v_ood_list, r_ood_list)
...
if __name__=='__main__':
    ...
    data_dir = pathlib.Path(f'/mnt/bn/bykang/phy-data/simple_scenes_data/pedulum')
    if args.data_for_vis:
        data_name = 'vis_data_L'
        for direction in ['left', 'upper', 'down']:
            if direction in args.data_name:
                data_name = f'vis_data_L_{direction}'
                DIRECTION = direction
        ...
        args.data_dir = data_dir / f'data_{size}_{args.data_name}'
        if not os.path.exists(args.data_dir):
...
# sudo /mnt/bn/magic/yueyang/miniconda3/envs/phyre/bin/python3 one_ball_pedulum.py  --data_for_vis
# sudo /mnt/bn/magic/yueyang/miniconda3/envs/phyre/bin/python3 one_ball_pedulum.py  --data_for_vis --data_name right
# sudo /mnt/bn/magic/yueyang/miniconda3/envs/phyre/bin/python3 one_ball_pedulum.py --data_name in_dist_v2 --data_size_level 0 --num_workers 64
# sudo /mnt/bn/magic/yueyang/miniconda3/envs/phyre/bin/python3 one_ball_pedulum.py --data_name in_dist_v2 --data_size_level 1 --num_workers 64
# sudo /mnt/bn/magic/yueyang/miniconda3/envs/phyre/bin/python3 one_ball_pedulum.py --data_name in_dist_v2 --data_size_level 2 --num_workers 64