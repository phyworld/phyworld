...
def in_dist_generate():
    ...
    low_r_list = np.linspace(MIN_R, MID_R, R_SPACE//2)
    high_r_list = np.linspace(MID_R, MAX_R, R_SPACE//2)

    # List to hold all combinations
    combinations1 = generate_combinations(low_r_list, v_list, color='red')
    combinations2 = generate_combinations(high_r_list, v_list, color='blue')
...
def eval_dist_generate():
    ...
    v_list = np.linspace(MIN_V, MAX_V, V_SPACE) # 1-4
    low_r_list = np.linspace(MIN_R, MID_R, R_SPACE) # 0.7-0.8
    high_r_list = np.linspace(MID_R, MAX_R, R_SPACE) # 1.3-1.4
    ...
    # combinations2 = generate_combinations(high_r_list, v_list, color='blue')
    # eval
    combinations1 = generate_combinations(low_r_list, v_list, color='blue')
    combinations2 = generate_combinations(high_r_list, v_list, color='red')
    combinations3 = generate_combinations(low_r_list, v_list, color='red')
    combinations4 = generate_combinations(high_r_list, v_list, color='blue')
...
if __name__=='__main__':
    ...
    if not args.eval:
        ...
        args.data_dir = data_dir / f'color_size_{size}'
    else:
        # eval data
        combinations = eval_dist_generate()
        size = get_size_str(len(combinations))
        args.data_dir = data_dir / f'color_size_eval_{size}'
    print(args.data_dir)
...
# python3 one_ball_uniform_motion_color_size.py --data_for_vis --num_workers 1
# python3 one_ball_uniform_motion_color_size.py --data_size_level 1 --num_workers 64
# python3 one_ball_uniform_motion_color_size.py --data_size_level 1 --eval --num_workers 32