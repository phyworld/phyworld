...
def simulate_parabola(
    ...
    if ball_height is None: 
        ...
        ball_height = uniform_random(
            (1 - eps) * world_scale - max_r - 1.0,
            (1 - eps) * world_scale - max_r,
        )

    world = b2World(gravity=(0, -2.5), doSleep=False)
...
def generate_videos_hdf5(scenes, scenes_id, data_dir='./', seed=42, store_video=False):
    ...
    for scene in scenes:
        r1, v1 = scene
        frames, positions, success = simulate_parabola(v1, r1, world_scale=WORLD_SCALE, stride=STRIDE, numsteps=STRIDE*NUM_FRAMES)
...
if __name__=='__main__':
    ...
    data_dir = pathlib.Path(f'/mnt/bn/bykang/phy-data/simple_scenes_data/parabola')
