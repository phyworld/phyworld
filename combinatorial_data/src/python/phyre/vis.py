...
def save_observation_series_to_gif_with_space(batched_observation_series,
                                   fpath,
                                   solved_states=None):
    """Saves a list of arrays of intermediate scenes as a gif."""
    max_steps = max(len(img) for img in batched_observation_series)
...
def save_mp4(images, fps, fpath):
    imgs = observations_to_uint8_rgb(images)
    imgs = imgs[::-1, ::-1] # reverse time and height
    imageio.mimsave(fpath, imgs, fps=fps)
    
def observations_to_frames(images):
    imgs = observations_to_uint8_rgb(images)
    return imgs[::-1, ::-1] # reverse height

def batched_observations_to_frames(batched_images):
    # torch is 5x faster than numpy even on CPU
    batched_images = torch.tensor(np.stack(batched_images, 0), dtype=torch.long)
    batched_images = WAD_COLORS_TORCH[batched_images]
    return batched_images.numpy()[:, :, ::-1] # reverse height
